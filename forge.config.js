...
module.exports = {
	...
	electronWinstallerConfig: {
		...
		iconUrl:
			"https://raw.githubusercontent.com/adenvt/recta-host/develop/src/img/icons/win/icon.ico",
	...
	electronInstallerDebian: {
		...
		rename(dest, src) {
			return path.join(
				dest,
				`recta-host-${platform}-${arch}-${version}.deb`
			);
		}
	},
	electronInstallerRedhat: {
		...
		rename(dest, src) {
			return path.join(
				dest,
				`recta-host-${platform}-${arch}-${version}.rpm`
			);
		}
	},
	github_repository: {
